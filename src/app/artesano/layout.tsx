...
const NAV = [
  { href: '/artesano/dashboard', icon: '⊞', label: 'Dashboard' },
  { href: '/artesano/productos', icon: '🏺', label: 'Productos' },
  { href: '/artesano/raices',    icon: '📖', label: 'Raíces' },
  { href: '/artesano/cfo',       icon: '🤖', label: 'CFO-Bot' },
  { href: '/artesano/academia',  icon: '🎓', label: 'Academia' },
  { href: '/artesano/ferias',    icon: '🎪', label: 'Ferias' },
  { href: '/artesano/talleres',  icon: '🎨', label: 'Talleres' },
  { href: '/artesano/scoring',   icon: '💰', label: 'Scoring' },
  { href: '/artesano/comunidad', icon: '🌐', label: 'Comunidad' },
  { href: '/artesano/grupos',    icon: '💬', label: 'Grupos' },
  { href: '/artesano/perfil',    icon: '👤', label: 'Perfil' },
]

export default function ArtesanoLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (status === 'authenticated' && session.user.role !== 'ARTESANO' && session.user.role !== 'ADMIN') {
      ...
      <aside className={`fixed md:sticky top-0 left-0 h-screen z-50 w-64 flex flex-col border-r border-[#F0EAE0]/[0.06] transition-transform duration-300 md:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}
        ...
        <div className="p-3 border-t border-[#F0EAE0]/[0.06]">
          <div className="flex items-center gap-3 px-3 py-2.5">
            <div className="w-8 h-8 rounded-full bg-[#3D1C02] flex items-center justify-center text-xs font-bold text-[#F0EAE0]">
              {session.user.phone?.slice(-2)}
            </div>
            <div className="flex-1 min-w-0">
              <div className="text-xs text-[#F0EAE0]/60 truncate">{session.user.phone}</div>
              <div className="text-[10px] text-[#C84B2F] font-medium">Artesano</div>
            </div>
