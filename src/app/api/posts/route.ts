...
export async function GET(req: Request) {
  ...
  const [posts, activeUsers] = await Promise.all([
    prisma.post.findMany({
      ...
      take: 50,
      include: {
        author: { select: { id: true, name: true, nickname: true, avatar_url: true, role: true } },
  ...
  return NextResponse.json({
    posts: posts.map((p) => ({
      ...
      author: { ...p.author, name: p.author.nickname || p.author.name },
...
export async function POST(req: Request) {
  ...
  const post = await prisma.post.create({
    ...
    include: { author: { select: { id: true, name: true, nickname: true, avatar_url: true, role: true } } },
  })

  return NextResponse.json(
    { ok: true, post: { ...post, author: { ...post.author, name: post.author.nickname || post.author.name } } },
    { status: 201 },
  )
}
